...
vars2split  = {'dprime', 'crit'};
statkinds   = {'ttest'}; % 'ttest', 'anova', 'glme'};
        ...
        for c = 1:length(conds),
            ...
            for e = 1:2,
                ...
                for sj = unique(dat.subj_idx)',
                    ...
                    % skip if this su
                    if all(all(isnan(dat.pupil_dilation_enc(idx, :)))),
                        continue;
                    end
                    
                    % median split
                    qntls   = nanmedian(thisdat.pupil_dilation_enc);
                    pupbins = discretize(thisdat.pupil_dilation_enc, ...
                        [1.1*min(thisdat.pupil_dilation_enc) qntls 1.1*max(thisdat.pupil_dilation_enc)]);
                    ...
                    switch vars2split{v}
                        case 'dprime'
                            tmpavgdat = array2table([sj*ones(2,1) [0; 1] dprimebins([1 end])], 'variablenames', ...
                                {'sj', 'split', 'pupil'});
                        case 'crit'
                            tmpavgdat = array2table([sj*ones(2,1) [0; 1] criterionbins([1 end])], 'variablenames', ...
                                {'sj', 'split', 'pupil'});
                    end
                ...
                switch statkinds{statidx}
                    ...
                    case 'ttest'
                        ...
                        % stats separately for each condition
                        if pval < 0.001,
                            txt{cnt} = sprintf('%s %s\nt(%d) = %.2f, p < 0.001', emtxt, condtxt, stats.df, stats.tstat);
                        else
                            txt{cnt} = sprintf('%s %s\nt(%d) = %.2f, p = %.3f', emtxt, condtxt, stats.df, stats.tstat, pval);
                        end
                        cnt = cnt + 1;
...
%         axis off;
        ...
        tightfig;
        print(gcf, '-dpdf', sprintf('%s/figures/scatter_SDT_figure2_%s_%s.pdf', ....
            mypath, vars2split{v}, statkinds{statidx}));
        clear txt avgdat
    end
end
end
